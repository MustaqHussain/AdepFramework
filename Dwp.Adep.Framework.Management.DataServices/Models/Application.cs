...
namespace Dwp.Adep.Framework.Management.DataServices.Models
{
    public partial class Application : IAuditable, IActiveAware 
    {
        ...
        public virtual ICollection<UploadQueue> UploadQueue
        {
            get
            {
                if (_uploadQueue == null)
                {
                    var newCollection = new FixupCollection<UploadQueue>();
                    newCollection.CollectionChanged += FixupUploadQueue;
                    _uploadQueue = newCollection;
                }
                return _uploadQueue;
            }
            set
            {
                ...
                {
                    var previousValue = _uploadQueue as FixupCollection<UploadQueue>;
                    if (previousValue != null)
                    {
                        previousValue.CollectionChanged -= FixupUploadQueue;
                    }
                    _uploadQueue = value;
                    var newValue = value as FixupCollection<UploadQueue>;
                    if (newValue != null)
                    {
                        newValue.CollectionChanged += FixupUploadQueue;
                    }
                }
            }
        }
        private ICollection<UploadQueue> _uploadQueue;
        ...
        private void FixupUploadQueue(object sender, NotifyCollectionChangedEventArgs e)
        {
            if (e.NewItems != null)
            {
                foreach (UploadQueue item in e.NewItems)
                {
                    item.Application = this;
                }
            }
            ...
            if (e.OldItems != null)
            {
                foreach (UploadQueue item in e.OldItems)
                {
                    if (ReferenceEquals(item.Application, this))
                    {
                        item.Application = null;
